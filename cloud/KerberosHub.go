...
func UploadKerberosHub(streamKey string, fileName string, config *config.Config) (bool, bool, error) {
	...
	log.Info("UploadKerberosHub: Uploading to Kerberos Hub (" + config.HubURI + ")")
	log.Info("UploadKerberosHub: Upload started for " + fileName)
		...
		if resp != nil {
			if err == nil {
				if resp.StatusCode == 200 {
					log.Info("UploadKerberosHub: Upload allowed using the credentials provided (" + config.HubKey + ", " + config.HubPrivateKey + ")")
				} else {
					log.Info("UploadKerberosHub: Upload NOT allowed using the credentials provided (" + config.HubKey + ", " + config.HubPrivateKey + ")")
	...
	if err == nil {
		if resp != nil {
			...
			if err == nil {
				if resp.StatusCode == 200 {
					log.Info("UploadKerberosHub: Upload Finished, " + resp.Status + ".")
					return true, true, nil
				} else {
					log.Info("UploadKerberosHub: Upload Failed, " + resp.Status + ", " + string(body))
					return false, true, nil
				}
			}
		}
